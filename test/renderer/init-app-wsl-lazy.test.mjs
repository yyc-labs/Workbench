...
test('initApp skips saved WSL project filesystem probes in windows-native mode', async () => {
  const originalWindow = globalThis.window
  const originalDocument = globalThis.document
  const originalLocalStorage = globalThis.localStorage
  ...
  const wslPath = '\\\\wsl.localhost\\Ubuntu\\home\\yyc\\repo'
  const windowsPath = 'D:\\tools\\native-repo'
  const detectCalls = []
  ...
  const state = {
    ...
    syncManagedProcesses: async () => undefined,
  }
  ...
  globalThis.window = {
    electronAPI: {
      ...
      detectProjects: async (path) => {
        detectCalls.push(path)
        if (path === wslPath) {
          throw new Error('WSL path should not be probed during native startup')
        }
        return {
          id: 'native-project',
          path,
  ...
  try {
    ...
    assert.deepEqual(detectCalls, [windowsPath])
    assert.equal(state.projects.length, 2)
    assert.equal(state.projects[0].path, wslPath)
    assert.equal(state.projects[0].name, 'repo')
    assert.equal(state.projects[0].type, 'unknown')
    assert.equal(state.projects[1].type, 'node')
  } finally {
    globalThis.window = originalWindow
    globalThis.document = originalDocument
    globalThis.localStorage = originalLocalStorage
