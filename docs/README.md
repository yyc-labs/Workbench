...
└── development/              # 开发过程资料，默认作为本地过程资料管理
    ├── visualizations/       # 轻量可视化运行框架和示例
    ├── plans/                # 需求、设计、优化和执行计划
...
`docs/development/` 默认用于保存不应进入公开仓库的过程性资料。大部分内容仍保留在本地，便于继续追踪历史上下文，但不会作为正式项目文档发布。`visualizations/` 是一个例外，它提供可复用的静态可视化工作台，因此会随仓库一并维护。
