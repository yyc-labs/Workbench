...
- P0 已完成：任务说明改为可选；学习笔记来源支持搜索、分类筛选、当前结果全选/清空和已选摘要；任务与来源同时为空时不会进入浏览器执行链路。
- P1 已完成：浏览器 AI 任务独立保存到 `userData/browser-ai/records`，支持列表、详情、重命名、删除、重新载入、复制回答和保存为学习笔记。
- P2 已完成：Playwright 执行事件携带步骤快照，renderer 显示执行时间线；等待回答期间按周期发送页面检查心跳，并记录到任务详情。
- P3 已完成：补充 context composer、任务记录 repository 和浏览器 AI 来源/步骤相关定向测试；已执行 `npm run typecheck` 和 `npm test`，未执行 build。
- 隐私策略按默认决策落地：任务记录保存回答、来源摘要、任务说明和执行结果；个人上下文原文默认不保存，只有发送窗口明确勾选完整发送内容时才保留来源正文。
- 追加默认配置体验：学习中心的“发送到浏览器 AI”按钮支持右键打开配置弹窗，可设置默认学习笔记、默认保存完整发送内容和默认目标站点；普通打开时仍可临时调整本次来源。
