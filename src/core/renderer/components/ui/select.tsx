  ...
  return (
    <div className={cn("relative", className)}>
      ...
      >
        <span className={cn(
          "min-w-0 truncate",
          !selectedOption && "text-[color:var(--color-muted-foreground)]"
        )}>
          {renderValue ? renderValue(selectedOption) : (selectedOption?.label ?? placeholder)}
        </span>
        <ChevronDown
          className={cn(
            "h-4 w-4 shrink-0 text-[color:var(--color-muted-foreground)] transition-transform duration-200",
            open && "rotate-180"
          )}
        />
      </button>
          ...
          <div className="overflow-auto" style={{ maxHeight: scrollAreaMaxHeight }}>
            {options.length > 0 ? options.map((option, index) => {
              ...
              return (
                ...
                >
                  {renderOption ? renderOption(option, { selected, highlighted }) : (
                    <>
