...
import type { ProjectDocTagOption, TranscriptSession, TranscriptSessionSummary, TranscriptViewerMode } from '../../../shared/types'
import type { InterpolationValues, MessageKey, ResolvedLocale, SettingsSectionMessageKey } from '../../i18n/messages'
...
function TranscriptModeButton({ active, disabled = false, icon, label, onClick }: { active: boolean; disabled?: boolean; icon: JSX.Element; label: string; onClick: () => void }) {
  return (
    <button
      type="button"
      className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-medium transition-colors ${active ? 'bg-primary text-white' : 'text-[color:var(--color-muted-foreground)] hover:bg-[color:var(--color-accent)] hover:text-[color:var(--color-foreground)]'}`}
      onClick={onClick}
      disabled={disabled}
      title={label}
    >
      ...
      {!projectHeaderCollapsed && (
        <header className="app-chrome pointer-events-auto absolute inset-x-0 top-0 z-[85] flex min-h-[84px] items-center justify-between gap-4 px-8 py-4">
          <div className="flex min-w-0 items-center gap-4">
            <button className="rounded-full p-2 text-[color:var(--color-muted-foreground)] transition-colors hover:bg-[color:var(--color-accent)] hover:text-[color:var(--color-foreground)]" onClick={() => navigateToPane('code')}>
            ...
            <div className="min-w-0">
              ...
              <p className="mt-0.5 text-[11px] text-[color:var(--color-muted-foreground)]/85">{t('transcript.pageTitle')}</p>
            </div>
          </div>
          ...
          <div className="flex shrink-0 items-center gap-3">
            ...
            />
            {renderProjectLinksButton()}
            <button type="button" className="inline-flex items-center gap-2 rounded-full border border-[color:var(--color-border)] px-4 py-2 text-sm font-medium text-[color:var(--color-foreground)] transition-colors hover:bg-[color:var(--color-accent)]" onClick={onOpenManualImport}>
              <FileText className="h-4 w-4" />
              {t('transcript.importPastedContent')}
            </button>
            <button
              type="button"
              className={`inline-flex h-9 items-center gap-1.5 rounded-full px-4 py-2 text-sm font-medium transition-all ${hasTerminalOutput ? 'bg-primary text-white shadow-sm hover:bg-primary-hover' : 'cursor-not-allowed border border-[color:var(--color-border)] text-[color:var(--color-muted-foreground)]'}`}
            ...
            >
              {isImporting ? <RefreshCw className="h-3.5 w-3.5 animate-spin" /> : <ArrowDownToLine className="h-3.5 w-3.5" />}
              {t('transcript.importCurrentOutput')}
            </button>
            <div className="ml-1 flex items-center gap-2">
              <span className={`hidden text-xs min-[1480px]:inline ${saveStatusToneClass}`}>{saveStatusText}</span>
              <Button type="button" className="h-9 rounded-full px-4" onClick={onSaveTranscript} disabled={saveButtonDisabled} title={t('transcript.saveShortcutHint')}>
        ...
        <div className="flex items-center justify-between gap-4">
            ...
            {projectHeaderCollapsed ? (
              ...
              <div className="min-w-0">
                ...
                <div className="mt-1 flex min-w-0 flex-wrap items-center gap-2">
                  <span className="inline-flex items-center rounded-full border border-[color:var(--color-border)] px-2.5 py-0.5 text-[11px] text-[color:var(--color-muted-foreground)]">{transcriptCountLabel}</span>
                  <span className="inline-flex items-center rounded-full border border-[color:var(--color-border)] px-2.5 py-0.5 text-[11px] text-[color:var(--color-muted-foreground)]">{projectDocsCountLabel}</span>
                  {session && (
                    <span className="inline-flex max-w-[280px] items-center truncate rounded-full border border-[color:var(--color-border)] px-2.5 py-0.5 text-[11px] text-[color:var(--color-muted-foreground)]" title={session.title}>
                      {session.title}
                    </span>
                  )}
                  {session && <span className="inline-flex items-center rounded-full border border-[color:var(--color-border)] px-2.5 py-0.5 text-[11px] text-[color:var(--color-muted-foreground)]">{formatTranscriptSourceType(locale, session.sourceType)}</span>}
                  {session && <span className="inline-flex items-center rounded-full border border-[color:var(--color-border)] px-2.5 py-0.5 text-[11px] text-[color:var(--color-muted-foreground)]">{t('transcript.refs', { count: session.references.length })}</span>}
                </div>
              </div>
            )}
          </div>
          ...
          <div className="flex min-w-0 shrink-0 items-center justify-end gap-3">
            {projectHeaderCollapsed ? (
              ...
              <>
                {session && <span className={`hidden text-xs min-[1240px]:inline ${saveStatusToneClass}`}>{saveStatusText}</span>}
                <button type="button" className="inline-flex h-8 w-8 items-center justify-center rounded-full text-[color:var(--color-muted-foreground)] transition-colors hover:bg-[color:var(--color-accent)] hover:text-[color:var(--color-foreground)]" onClick={onRefreshList} title={t('settingsTranscript.refresh')}>
...
export function TranscriptListSidebar({ listStatus, summaries, resolvedActiveTranscriptId, deletingTranscriptId, transcriptCountLabel, locale, formatDateTime, formatTranscriptSourceType, onRefreshList, onSelectTranscript, onDeleteTranscript, t }: TranscriptListSidebarProps) {
  ...
  useEffect(() => {
    setCollapsedGroupIds((current) => {
      ...
      const activeGroupId = sourceGroups.find((group) => group.summaries.some((summary) => summary.id === resolvedActiveTranscriptId))?.id
    ...
    <aside className="flex h-full w-full min-h-0 flex-col overflow-hidden rounded-[24px] border border-[color:var(--color-border)] bg-[color:var(--color-card)]">
      <div className="border-b border-[color:var(--color-border)] px-5 py-4">
        <div className="flex items-center justify-between gap-3">
          <div>
            <p className="text-sm font-semibold text-[color:var(--color-foreground)]">{t('transcript.listTitle')}</p>
            <p className="mt-1 text-xs text-[color:var(--color-muted-foreground)]">{transcriptCountLabel}</p>
          </div>
          <button type="button" className="inline-flex h-8 w-8 items-center justify-center rounded-full text-[color:var(--color-muted-foreground)] transition-colors hover:bg-[color:var(--color-accent)] hover:text-[color:var(--color-foreground)]" onClick={onRefreshList} title={t('settingsTranscript.refresh')}>
            <RefreshCw className="h-4 w-4" />
          </button>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-3 py-3">
        {listStatus === 'loading' && summaries.length <= 0 ? (
          <div className="flex h-full items-center justify-center text-xs text-[color:var(--color-muted-foreground)]">{t('transcript.loadingTranscripts')}</div>
          ...
          <div className="flex h-full flex-col items-center justify-center gap-3 px-4 text-center">
            ...
            <button type="button" className="inline-flex items-center gap-1.5 rounded-full border border-[color:var(--color-border)] px-3 py-1.5 text-xs text-[color:var(--color-foreground)] transition-colors hover:bg-[color:var(--color-accent)]" onClick={onRefreshList}>
          ...
          <div className="flex h-full flex-col items-center justify-center gap-3 px-5 text-center">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-[color:var(--color-muted-foreground)]">{t('transcript.firstTranscriptHint')}</p>
            </div>
          </div>
        ) : (
            ...
            {sourceGroups.map((group) => {
              ...
              return (
                <div key={group.id} className="min-w-0">
                  <button type="button" className="code-tree-row font-semibold text-[color:var(--color-muted-foreground)]" style={{ paddingLeft: 10, paddingRight: 10 }} aria-expanded={!isCollapsed} onClick={() => toggleSourceGroup(group.id)} title={group.label}>
                    {isCollapsed ? <ChevronRight className="h-3.5 w-3.5 shrink-0 text-[color:var(--color-muted-foreground)]" /> : <ChevronDown className="h-3.5 w-3.5 shrink-0 text-[color:var(--color-muted-foreground)]" />}
                    {isCollapsed ? <Folder className="h-4 w-4 shrink-0 text-[color:var(--color-warning)]" /> : <FolderOpen className="h-4 w-4 shrink-0 text-[color:var(--color-warning)]" />}
                    <span className="block min-w-0 flex-1 truncate">{group.label}</span>
                    <span className="shrink-0 font-mono text-[10px] font-medium text-[color:var(--color-muted-foreground)]">{group.summaries.length}</span>
                  </button>
      ...
      {!resolvedActiveTranscriptId ? (
        <div className="flex h-full flex-col items-center justify-center gap-3 px-6 text-center">
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-[color:var(--color-muted-foreground)]">{t('transcript.selectOrImportHint')}</p>
          </div>
        </div>
      ) : !session ? (
        <div className="flex h-full items-center justify-center text-sm text-[color:var(--color-muted-foreground)]">{t('transcript.loadingTranscript')}</div>
      ) : (
        <>
            ...
            <div className="flex flex-col gap-4 min-[960px]:flex-row min-[960px]:items-start min-[960px]:justify-between">
              <div className="min-w-0 min-[960px]:max-w-[min(100%,560px)]">
                <div className="flex min-w-0 items-center gap-2">
                  <h2 className="min-w-0 max-w-full flex-1 truncate whitespace-nowrap text-lg font-semibold text-[color:var(--color-foreground)] min-[960px]:max-w-[300px]">{session.title}</h2>
                  <span className="shrink-0 rounded-full border border-[color:var(--color-border)] px-2.5 py-0.5 text-[11px] text-[color:var(--color-muted-foreground)]">{formatTranscriptSourceType(locale, session.sourceType)}</span>
                  <span className="shrink-0 rounded-full border border-[color:var(--color-border)] px-2.5 py-0.5 text-[11px] text-[color:var(--color-muted-foreground)]">{t('transcript.refs', { count: session.references.length })}</span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="quiet-control inline-flex items-center gap-1 rounded-full border border-[color:var(--color-border)] p-1">
                  <TranscriptModeButton active={effectiveMode === 'preview'} icon={<Eye className="h-3.5 w-3.5" />} label={t('transcript.preview')} onClick={() => setTranscriptMode(session.id, 'preview')} />
                  <TranscriptModeButton active={effectiveMode === 'editor'} icon={<Code2 className="h-3.5 w-3.5" />} label={t('transcript.editor')} onClick={() => setTranscriptMode(session.id, 'editor')} />
                  <TranscriptModeButton active={effectiveMode === 'split'} disabled={isNarrowViewport} icon={<Columns2 className="h-3.5 w-3.5" />} label={t('transcript.split')} onClick={() => setTranscriptMode(session.id, 'split')} />
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="min-h-0 flex-1 overflow-hidden p-4">
            <div className={`grid h-full min-h-0 overflow-hidden rounded-[20px] border border-[color:var(--color-border)] bg-[color:var(--color-background-subtle)] ${effectiveMode === 'split' ? 'min-[960px]:grid-cols-2' : 'grid-cols-1'}`}>
              {(effectiveMode === 'preview' || effectiveMode === 'split') && (
                <div ref={previewScrollRef} className="code-markdown-preview-scroll-root transcript-markdown-preview-scroll-root min-h-0 overflow-y-auto bg-[color:var(--color-card)]">
                  <article className="code-markdown-content code-markdown-content--viewport-scroll transcript-markdown-content px-6 py-6">
                    <MarkdownPreviewVisibilityProvider forceRenderAllBlocks>
                      <ReactMarkdown remarkPlugins={[remarkGfm, remarkBoxDrawingTables]} components={markdownComponents} urlTransform={transformMarkdownUrl}>
                        {displayMarkdownText}
                      </ReactMarkdown>
              ...
              {(effectiveMode === 'editor' || effectiveMode === 'split') && (
                <div className={`min-h-0 bg-[color:var(--color-card)] ${effectiveMode === 'split' ? 'border-t border-[color:var(--color-border)] min-[960px]:border-l min-[960px]:border-t-0' : ''}`}>
                  <MonacoTextViewer value={editorValue} filePath={`transcript/${session.id}.md`} language={inferLanguageFromRelativePath('transcript.md')} readOnly={false} onChange={setEditorValue} onSave={onSaveTranscript} modelNamespace="transcript-viewer" stickyScroll />
                </div>
              )}
            </div>
          </div>
