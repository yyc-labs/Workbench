...
import { type MarkdownCodeBlockExpandPayload, type MarkdownStructuredBlockClickPayload } from '../code/code.markdown'
...
function CaptureButton({ label, status, onCapture }: { label: string; status: CaptureState['status']; onCapture: () => void }) {
  return (
    <button
      type="button"
      className={`quiet-control inline-flex h-8 items-center gap-1.5 rounded-full border-0 px-3 text-xs transition-colors disabled:cursor-not-allowed disabled:opacity-60 ${
        status === 'success' ? 'text-[color:var(--color-success)]' : status === 'error' ? 'text-[color:var(--color-destructive)]' : 'text-[color:var(--color-foreground)]'
      }`}
      onClick={onCapture}
      title={label}
      disabled={status === 'running'}
    >
      {status === 'running' ? <RefreshCw className="h-3.5 w-3.5 animate-spin" /> : status === 'success' ? <Check className="h-3.5 w-3.5" /> : <Copy className="h-3.5 w-3.5" />}
      <span>{label}</span>
    </button>
  )
}
...
export function TranscriptPreviewModals({
  ...
  return (
    <>
      <ModalShell open={Boolean(structuredPreview)} onClose={onCloseStructuredPreview} widthClassName="max-w-[min(1280px,calc(100vw-40px))]" baseZIndex={1180} ariaLabel={t('transcript.structuredPreview')} overlayClassName="backdrop-blur-0 bg-black/18" panelClassName="transcript-structured-preview-modal p-4 sm:p-5">
        <div className="relative flex h-full min-h-0 flex-col">
          <div className="mb-4 flex items-start justify-between gap-4">
            <div className="min-w-0">
              <p className="section-label mb-1">{t('transcript.listTitle')}</p>
              <div className="flex flex-wrap items-center gap-2">
                <p className="text-sm font-semibold text-[color:var(--color-foreground)]">{structuredPreview ? formatStructuredBlockKindLabel(locale, structuredPreview.kind) : formatStructuredBlockKindLabel(locale, 'default')}</p>
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <button type="button" className="flex h-8 w-8 items-center justify-center rounded-full text-[color:var(--color-muted-foreground)] transition-colors hover:bg-[color:var(--color-accent)] hover:text-[color:var(--color-foreground)]" onClick={onCloseStructuredPreview} title={t('common.close')}>
                <X className="h-4 w-4" />
              </button>
            </div>
          </div>

          <ZoomPanViewport captureTargetRef={structuredPreviewCapture.targetRef} resetKey={structuredPreviewMarkdown}>
            <div data-capture-surface="structured-preview" className="transcript-preview-zoom-capture">
              <StructuredPreviewMarkdown contentRef={structuredPreviewCapture.contentRef} markdown={structuredPreviewMarkdown} components={structuredPreviewComponents} />
            </div>
          </ZoomPanViewport>
        </div>
      </ModalShell>

      <ModalShell open={Boolean(codePreview)} onClose={onCloseCodePreview} widthClassName="max-w-[min(1280px,calc(100vw-40px))]" baseZIndex={1180} ariaLabel={t('codeMarkdown.previewAria')} overlayClassName="backdrop-blur-0 bg-black/18" panelClassName="code-markdown-code-preview-modal p-4 sm:p-5">
        <div className="relative flex max-h-[min(88vh,980px)] min-h-0 flex-col">
          <div className="mb-4 flex items-start justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <button type="button" className="flex h-8 w-8 items-center justify-center rounded-full text-[color:var(--color-muted-foreground)] transition-colors hover:bg-[color:var(--color-accent)] hover:text-[color:var(--color-foreground)]" onClick={onCloseCodePreview} title={t('common.close')}>
                <X className="h-4 w-4" />
              </button>
            </div>
          </div>
