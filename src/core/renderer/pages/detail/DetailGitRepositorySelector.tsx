...
function repositoryRootLabel(repo: DetailGitRepositorySummary, t: (key: string, values?: Record<string, number | string>) => string): string {
  return repo.isGitRepository === false ? t('detail.repositorySelectorRootNotGit') : t('detail.repositorySelectorRoot')
}

function repositoryLabel(repo: DetailGitRepositorySummary, t: (key: string, values?: Record<string, number | string>) => string): string {
  return repo.relativePath === '.' ? `${repo.name} · ${repositoryRootLabel(repo, t)}` : `${repo.relativePath} · ${repo.isNested ? t('detail.repositorySelectorNested') : t('detail.repositorySelectorRepository')}`
}

export function DetailGitRepositorySelector({ repositories, selectedRepositoryId, snapshot, loading, repositoriesLoading, repositoriesTruncated, variant = 'section', onChangeRepository, onRefreshRepositories }: DetailGitRepositorySelectorProps) {
  const { t } = useI18n()
  ...
  const statusText = loading ? t('detail.repositorySelectorLoading') : snapshot?.isGitRepository ? `${branchName} · ${t('detail.repositorySelectorChanges', { count: changeCount })}` : t('detail.repositorySelectorNoRepository')
  ...
  const controls = (
    <div className="flex min-w-0 items-center gap-2">
      ...
      <Combobox
        ...
        inputClassName={`h-${variant === 'inline' ? '7' : '8'} rounded-full pl-3 pr-11 text-xs`}
        triggerClassName={`${controlHeightClass} ${buttonMaxWidthClass} bg-[color:var(--color-card)]/72 pl-3 pr-11 text-xs hover:border-[color:var(--color-border-hover)] hover:bg-[color:var(--color-popover)]/82`}
        contentClassName="w-[min(360px,calc(100vw-32px))]"
        renderDisplayValue={() => <span className="min-w-0 flex-1 truncate text-[color:var(--color-foreground)]">{selectedRepository ? repositoryLabel(selectedRepository, t) : t('detail.repositorySelectorNoRepository')}</span>}
            ...
            <>
              ...
              <span className="min-w-0 flex-1">
                ...
                <span className="block truncate text-[10.5px] text-[color:var(--color-muted-foreground)]">
                  {repo.relativePath === '.' ? repositoryRootLabel(repo, t) : repo.isNested ? t('detail.repositorySelectorNested') : t('detail.repositorySelectorRepository')}
                  {' · '}
                  {repo.repoRoot}
                </span>
              </span>
        ...
        filterOption={(option, nextQuery) => {
          ...
          return repo.name.toLowerCase().includes(normalizedQuery) || repo.relativePath.toLowerCase().includes(normalizedQuery) || repo.repoRoot.toLowerCase().includes(normalizedQuery)
        }}
        onOpenChange={(isOpen) => {
          if (isOpen) {
  ...
  if (variant === 'inline') {
    return <div className="flex min-w-0 items-center">{controls}</div>
  }
        ...
        <div className="flex min-w-[240px] flex-1 items-center gap-2">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2">
              ...
              {repositoriesTruncated && <span className="rounded-full bg-[color:var(--color-warning-background)] px-2 py-0.5 text-[10px] text-[color:var(--color-warning)]">{t('detail.repositorySelectorTruncated')}</span>}
            </div>
