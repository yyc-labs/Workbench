...
export function LearningEditorPanel({
  ...
  const editorPreviewGridColumns = editorDisplayMode === 'preview' ? 'minmax(0,1fr)' : 'minmax(0,1fr) minmax(0,1fr)'

  return (
    <Card className={`min-h-0 overflow-hidden border-[color:var(--color-border)]/80 bg-[color:var(--color-card)]/94 ${bothSidebarsCollapsed ? 'mx-auto w-full max-w-[1360px]' : ''}`}>
      {selectedNote ? (
        <div className="flex h-full min-h-0 flex-col">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[color:var(--color-border)] px-5 py-4">
            <div className="min-w-0">
              <div className="line-clamp-1 text-sm font-semibold text-[color:var(--color-foreground)]">{editorTitle || t('learning.editor.untitledNote')}</div>
              <div className="text-xs text-[color:var(--color-muted-foreground)]">{editorDisplayMode === 'preview' ? t('learning.editor.previewModeDescription') : t('learning.editor.splitModeDescription')}</div>
            </div>
          ...
          <div className="grid h-full min-h-0" style={{ gridTemplateColumns: editorPreviewGridColumns }}>
            {editorDisplayMode === 'split' ? (
              <div className="flex min-h-0 flex-col border-r border-[color:var(--color-border)]">
                <div className="border-b border-[color:var(--color-border)] px-5 py-4">
                  ...
                  <div className="text-xs text-[color:var(--color-muted-foreground)]">{t('learning.editor.editDescription')}</div>
                </div>
            ...
            <div className="flex min-h-0 flex-col">
              <div className="border-b border-[color:var(--color-border)] px-5 py-4">
                ...
                <div className="text-xs text-[color:var(--color-muted-foreground)]">{editorDisplayMode === 'preview' ? t('learning.editor.previewOnlyDescription') : t('learning.editor.previewDescription')}</div>
              </div>
              <ScrollArea
              ...
              >
                <article className={`code-markdown-content code-markdown-content--viewport-scroll ${editorDisplayMode === 'preview' ? 'px-5 py-5 sm:px-6' : 'px-3 py-4 sm:px-4'}`} style={{ margin: 0, maxWidth: 'none', minWidth: 0, width: '100%' }}>
        ...
        <div className="flex h-full items-center justify-center text-sm text-[color:var(--color-muted-foreground)]">{t('learning.editor.emptyState')}</div>
      )}
    </Card>
  )
}
