...
export function LearningFrontmatterDialog({ categories, categoryInput, error, mode, open, status, submitting, tags, title, onCategoryInputChange, onClose, onStatusChange, onSubmit, onTagsChange, onTitleChange }: LearningFrontmatterDialogProps) {
  const { t } = useI18n()
  ...
  const uncategorizedOption = useMemo<ComboboxOption[]>(() => [{ value: '', label: t('learning.frontmatter.uncategorized') }], [t])
  ...
  return (
    <ModalShell open={open} onClose={handleClose} widthClassName="max-w-[640px]" ariaLabel={mode === 'create' ? t('learning.frontmatter.createAria') : t('learning.frontmatter.editAria')} panelClassName="p-0 overflow-hidden">
      <div className="flex flex-col">
        <div className="border-b border-[color:var(--color-border)] px-5 py-5">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <div className="text-base font-semibold text-[color:var(--color-foreground)]">{mode === 'create' ? t('learning.frontmatter.createTitle') : t('learning.frontmatter.editTitle')}</div>
              <div className="mt-1 text-sm leading-6 text-[color:var(--color-muted-foreground)]">{mode === 'create' ? t('learning.frontmatter.createDescription') : t('learning.frontmatter.editDescription')}</div>
            </div>
            <button
              type="button"
        ...
        <div className="grid gap-4 px-5 py-5 sm:grid-cols-2">
          <div className="sm:col-span-2">
            ...
            <Input value={title} onChange={(event) => onTitleChange(event.target.value)} placeholder={t('learning.frontmatter.titlePlaceholder')} disabled={submitting} />
          </div>
          <div>
            ...
            <Combobox
              ...
              disabled={submitting}
            />
            <div className="mt-2 text-xs text-[color:var(--color-muted-foreground)]">{t('learning.frontmatter.categoryHint')}</div>
          </div>
          <div>
            <div className="mb-1.5 text-xs text-[color:var(--color-muted-foreground)]">{t('learning.frontmatter.statusLabel')}</div>
            <Select ariaLabel="learning-frontmatter-status" value={status} options={statusOptions} onChange={(value) => onStatusChange(value as LearningNoteStatus)} disabled={submitting} />
          </div>
          <div className="sm:col-span-2">
            <div className="mb-1.5 text-xs text-[color:var(--color-muted-foreground)]">{t('learning.frontmatter.tagsLabel')}</div>
            <Input value={tags} onChange={(event) => onTagsChange(event.target.value)} placeholder={t('learning.frontmatter.tagsPlaceholder')} disabled={submitting} />
            <div className="mt-2 text-xs text-[color:var(--color-muted-foreground)]">{t('learning.frontmatter.tagsHint')}</div>
          </div>
          {error ? <div className="sm:col-span-2 text-sm text-[color:var(--color-destructive)]">{error}</div> : null}
        </div>

        <div className="flex justify-end gap-3 border-t border-[color:var(--color-border)] px-5 py-4">
          <Button variant="outline" className="rounded-full" onClick={handleClose} disabled={submitting}>
            {t('common.cancel')}
          </Button>
          <Button className="rounded-full" onClick={() => void onSubmit()} loading={submitting}>
