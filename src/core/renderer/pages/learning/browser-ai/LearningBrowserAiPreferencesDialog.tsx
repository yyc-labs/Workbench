...
import { readLearningBrowserAiPreferences, saveLearningBrowserAiPreferences } from './learningBrowserAiPreferences'
...
export function LearningBrowserAiPreferencesDialog({ open, notes, skills, categories, onClose }: LearningBrowserAiPreferencesDialogProps) {
  ...
  const siteOptions = useMemo(() => snapshot?.config.sites.map((site) => ({ value: site.id, label: `${site.name} · ${site.url}` })) ?? [], [snapshot?.config.sites])
    ...
    <ModalShell open={open} onClose={onClose} widthClassName="max-w-2xl" panelClassName="!p-0 flex max-h-[calc(100vh-3rem)] flex-col overflow-hidden" ariaLabel={t('learning.browserAi.preferencesTitle')}>
      <div className="flex items-start justify-between gap-4 border-b p-5" style={{ borderColor: 'var(--color-border)' }}>
        <div>
          <p className="section-label mb-2">
            <Settings2 className="mr-1 inline-block h-3.5 w-3.5" />
            {t('learning.browserAi.preferencesKicker')}
          </p>
      ...
      <div className="min-h-0 space-y-5 overflow-y-auto p-5">
        <section className="space-y-3">
          ...
          <LearningBrowserAiSourceSelector notes={notes} categories={categories} selectedNoteIds={selectedNoteIds} onSelectedNoteIdsChange={setSelectedNoteIds} />
        </section>
          ...
          <div className="space-y-1.5 rounded-[14px] border p-2" style={{ borderColor: 'var(--color-border)' }}>
            {skills
              ...
              .map((skill) => {
                ...
                return (
                  <label key={skill.id} className="flex cursor-pointer items-start gap-2 rounded-[10px] px-2.5 py-2 text-xs hover:bg-[color:var(--color-accent)]">
                    <Checkbox checked={checked} onChange={() => setSelectedSkillIds((current) => (checked ? current.filter((id) => id !== skill.id) : [...current, skill.id]))} />
                    <span className="min-w-0 flex-1">
                      <span className="block truncate text-[color:var(--color-foreground)]">{skill.title}</span>
                      <span className="mt-0.5 block truncate text-[color:var(--color-muted-foreground)]">{skill.tags.join(' · ') || skill.excerpt}</span>
                    </span>
                  </label>
                )
              })}
          ...
          {selectedSkillIds.length > 1 ? (
            <div className="flex flex-wrap gap-1.5">
              {selectedSkillIds.map((id, index) => (
                ...
                >
                  {index + 1}. {skills.find((skill) => skill.id === id)?.title}
                </Button>
              ))}
            </div>
          ) : null}
