...
export function LearningBrowserAiDialog({ open, notes, categories, skills, currentNote, initialRecord, onClose, onSaved }: LearningBrowserAiDialogProps) {
  ...
  return (
    <>
      <ModalShell open={open} onClose={onClose} widthClassName="max-w-4xl" panelClassName="!p-0 flex max-h-[calc(100vh-3rem)] flex-col overflow-hidden" ariaLabel={t('learning.browserAi.dialogTitle')}>
        <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
          ...
          <div className="min-h-0 space-y-4 overflow-y-auto p-5">
            <div className="grid gap-4 lg:grid-cols-2">
              <section className="space-y-3 rounded-[18px] border p-4" style={{ borderColor: 'var(--color-border)' }}>
                ...
                <div className="rounded-[14px] border p-3" style={{ borderColor: 'var(--color-border)' }}>
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div>
                      <div className="text-xs font-medium text-[color:var(--color-foreground)]">{t('learning.skills.selectSkill')}</div>
                      <div className="mt-1 text-[11px] text-[color:var(--color-muted-foreground)]">{t('learning.skills.selectedCount', { value: String(selectedSkillIds.length + (temporarySkill ? 1 : 0)) })}</div>
                    </div>
                  ...
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {selectedSkillIds.map((id) => (
                      <span key={id} className="rounded-full bg-[color:var(--color-primary)]/10 px-2.5 py-1 text-[11px] text-[color:var(--color-foreground)]">
                        {skills.find((skill) => skill.id === id)?.title ?? id}
                        {defaultSkillIds.includes(id) ? ` · ${t('learning.skills.sourceDefault')}` : ` · ${t('learning.skills.sourceThisTask')}`}
                      </span>
                    ))}
                    {temporarySkill ? (
                      <span className="rounded-full bg-[color:var(--color-accent)] px-2.5 py-1 text-[11px] text-[color:var(--color-foreground)]">
                        {temporarySkill.title} · {t('learning.skills.sourceTemporary')}
                      </span>
                    ) : null}
                  </div>
                </div>
                <label className="flex items-start gap-2 text-sm text-[color:var(--color-foreground)]">
                  <Checkbox
                    ...
                    onChange={(event) => {
                      setIncludePersonalContext(event.target.checked)
                      setPreview(null)
                    }}
                  />
                ...
                {includePersonalContext ? (
                  <Textarea
                    ...
                    onChange={(event) => {
                      setPersonalContext(event.target.value)
                      setPreview(null)
                    }}
                    placeholder={t('learning.browserAi.personalPlaceholder')}
                  />
                ) : null}
              ...
              <section className="space-y-3 rounded-[18px] border p-4" style={{ borderColor: 'var(--color-border)' }}>
                ...
                <Textarea
                  ...
                  placeholder={t('learning.browserAi.taskPlaceholder')}
                />
                <Input
                  ...
                  placeholder={t('learning.browserAi.responseFormatPlaceholder')}
                />
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <Button variant="outline" onClick={() => void preparePreview()} loading={loading && !isRunning} disabled={!canPrepare}>
                    <ExternalLink />
                    {t('learning.browserAi.previewAction')}
                  </Button>
            ...
            {snapshot?.connection === 'needs-login' ? (
              <div className="flex flex-wrap items-center gap-3 rounded-[14px] bg-[color:var(--color-accent)]/45 px-4 py-3 text-sm text-[color:var(--color-foreground)]">
                {t('learning.browserAi.loginRequired')}
                <Button variant="outline" size="sm" onClick={() => void openLogin()}>
                  {t('learning.browserAi.openLogin')}
                </Button>
              </div>
            ) : null}
          ...
          <div className="flex justify-end gap-2 border-t p-4" style={{ borderColor: 'var(--color-border)' }}>
            ...
            <Button onClick={() => void handleRun()} loading={loading} disabled={isRunning || !canPrepare}>
              <Send />
              {t('learning.browserAi.send')}
            </Button>
          </div>
        </div>
      </ModalShell>
      <SkillPickerDialog
        ...
        onTemporarySkillChange={(skill) => {
          setTemporarySkill(skill)
          setPreview(null)
        }}
