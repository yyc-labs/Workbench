...
export function LearningBrowserAiSourceSelector({ notes, categories, selectedNoteIds, onSelectedNoteIdsChange }: LearningBrowserAiSourceSelectorProps) {
  const { t } = useI18n()
  ...
  const toggleNote = (noteId: string, checked: boolean) => {
    onSelectedNoteIdsChange(checked ? Array.from(new Set([...selectedNoteIds, noteId])) : selectedNoteIds.filter((id) => id !== noteId))
  }
    ...
    <div className="space-y-3">
      <div className="flex flex-wrap items-center gap-2">
        <div className="relative min-w-[180px] flex-1">
          ...
          <Input className="h-9 pl-9 text-xs" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t('learning.browserAi.noteSearchPlaceholder')} aria-label={t('learning.browserAi.noteSearchPlaceholder')} />
        </div>
        <Select ariaLabel={t('learning.browserAi.noteCategoryFilter')} className="min-w-[150px] flex-1 sm:flex-none" value={categoryId} options={[{ value: 'all', label: t('learning.browserAi.allCategories') }, ...categories.map((category) => ({ value: category.id, label: category.name }))]} onChange={setCategoryId} />
      </div>
      <div className="flex flex-wrap items-center justify-between gap-2">
        <span className="text-xs text-[color:var(--color-muted-foreground)]">{t('learning.browserAi.selectedNotesCount', { value: String(selectedNoteIds.length) })}</span>
