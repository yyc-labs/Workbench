...
export function CreateSkillDialog({
	...
	return (
		<ModalShell
			...
			ariaLabel={t("learning.skills.createTitle")}
		>
			<div
			...
			>
				<div>
					...
					<h2 className="text-lg font-semibold text-[color:var(--color-foreground)]">
						{t("learning.skills.createTitle")}
					</h2>
					<p className="mt-1 text-xs text-[color:var(--color-muted-foreground)]">
						{t("learning.skills.createDescription")}
					</p>
				</div>
				<Button
					variant="ghost"
			...
			<div className="min-h-0 space-y-4 overflow-y-auto p-5">
				<label className="block space-y-1.5">
					<span className="text-xs font-medium text-[color:var(--color-muted-foreground)]">
						{t("learning.skills.titleLabel")}
					</span>
					<Input
						value={title}
						onChange={(event) => setTitle(event.target.value)}
						placeholder={t("learning.skills.titlePlaceholder")}
					/>
				</label>
				<label className="block space-y-1.5">
					<span className="text-xs font-medium text-[color:var(--color-muted-foreground)]">
						{t("learning.skills.tagsLabel")}
					</span>
					<Input
						value={tagInput}
						onChange={(event) => setTagInput(event.target.value)}
						placeholder={t("learning.skills.tagsPlaceholder")}
					/>
				</label>
				<label className="block space-y-1.5">
					<span className="text-xs font-medium text-[color:var(--color-muted-foreground)]">
						{t("learning.skills.contentLabel")}
					</span>
					<Textarea
						className="min-h-48 font-mono text-xs leading-6"
						value={contentMd}
						onChange={(event) => setContentMd(event.target.value)}
						placeholder={t("learning.skills.contentPlaceholder")}
					/>
				</label>
				<label className="flex items-start gap-2 text-sm text-[color:var(--color-foreground)]">
					<Checkbox
						checked={saveToSkills}
						onChange={(event) => setSaveToSkills(event.target.checked)}
					/>
					<span>
						{saveToSkills
							? t("learning.skills.saveToSkills")
							: t("learning.skills.useOnce")}
					</span>
				</label>
				{error ? (
