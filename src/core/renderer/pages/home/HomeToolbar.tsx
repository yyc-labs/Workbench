import { Search, Settings, Plus, Zap, SlidersHorizontal, X, BookOpenText, FileType2 } from 'lucide-react'
  ...
  return (
    <header className="app-chrome h-auto min-h-[68px] flex items-center px-8 py-3 gap-5 shrink-0">
      <div className="mr-5 flex shrink-0 items-center gap-3">
        ...
        <span className="whitespace-nowrap text-[15px] font-medium text-[color:var(--color-foreground)]">{t('common.runtime')}</span>
      </div>

      <div className="relative min-w-[220px] flex-1 max-w-[360px] xl:max-w-[420px]">
        <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[color:var(--color-muted-foreground)] pointer-events-none" strokeWidth={1.8} />
        <Input
          ref={searchRef}
          value={searchQuery}
      ...
      <div className="quiet-control ml-auto flex items-center rounded-full px-1.5 py-1 gap-2.5">
        <div className="flex items-center gap-2">
          ...
          <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full text-[color:var(--color-muted-foreground)] hover:text-[color:var(--color-foreground)] hover:bg-[color:var(--color-accent)]" onClick={onSettingsClick}>
