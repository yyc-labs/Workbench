...
function normalizeResponseTimeout(value: unknown): number {
  ...
  return Math.min(
    ...
    Math.max(MIN_RESPONSE_TIMEOUT_MS, Math.trunc(Number(value))),
  )
}
...
export function normalizeBrowserAiConfig(input: unknown): BrowserAiConfig {
  ...
  const edgeExecutablePath = typeof raw.edgeExecutablePath === 'string' && raw.edgeExecutablePath.trim()
    ? resolve(raw.edgeExecutablePath.trim())
    : DEFAULT_BROWSER_AI_CONFIG.edgeExecutablePath
  const legacySiteUrl = typeof raw.siteUrl === 'string' && raw.siteUrl.trim()
    ? raw.siteUrl.trim()
    : DEFAULT_BROWSER_AI_SITE_URL
  const legacySite = raw.site === 'chatgpt-web' && isSupportedChatGptSiteUrl(legacySiteUrl)
    ? 'chatgpt-web'
    : 'generic-web'
  const sites = normalizeBrowserAiSites(raw.sites, legacySiteUrl, legacySite)
  const activeSiteId = typeof raw.activeSiteId === 'string' && sites.some((site) => site.id === raw.activeSiteId)
    ? raw.activeSiteId
    : sites[0].id
  ...
  return {
    ...
    keepBrowserRunning: typeof raw.keepBrowserRunning === 'boolean'
      ? raw.keepBrowserRunning
      : DEFAULT_BROWSER_AI_CONFIG.keepBrowserRunning,
...
function normalizeBrowserAiSites(
  input: unknown,
  legacySiteUrl: string,
  legacySite: BrowserAiSiteProfile['site'],
): BrowserAiSiteProfile[] {
  ...
  const sites = rawSites.flatMap((value, index): BrowserAiSiteProfile[] => {
    ...
    const name = typeof raw.name === 'string' && raw.name.trim()
      ? raw.name.trim()
      : index === 0
        ? legacySite === 'chatgpt-web' ? 'ChatGPT' : DEFAULT_BROWSER_AI_SITE_NAME
        : `Web AI ${index + 1}`
    const site = raw.site === 'chatgpt-web'
      || (typeof raw.site !== 'string' && isSupportedChatGptSiteUrl(url))
      ? 'chatgpt-web'
      : 'generic-web'
...
export function resolveBrowserAiProfilePath(userDataPath: string): string {
  return join(resolve(userDataPath), 'browser-ai', 'edge-profile')
}
...
export function resolveEdgeExecutablePath(
  ...
  const candidates = [
    ...
    ...getDefaultEdgeExecutablePaths(env),
  ]
