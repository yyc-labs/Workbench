...
function composerLocators(page: Page): Locator[] {
  return [
    page.getByRole('textbox', { name: /message|prompt|ask|chat|question/i }),
...
async function isGenerating(page: Page): Promise<boolean> {
  return Boolean(await findVisibleLocator(page, [
    page.getByRole('button', { name: /stop generating|stop streaming|stop response|cancel response/i }),
...
async function lastAssistantText(page: Page): Promise<string> {
  const candidates = [
    ...
    page.locator('[data-author="assistant"]'),
    page.locator('[class*="assistant" i]'),
    page.locator('[class*="response" i]'),
...
function matchesConfiguredSite(url: string, configuredUrl?: string): boolean {
  if (!isSupportedBrowserAiSiteUrl(url)) return false
  if (!configuredUrl || !isSupportedBrowserAiSiteUrl(configuredUrl)) return true
  ...
  try {
    ...
    return actualHost === configuredHost || actualHost.endsWith(`.${configuredHost}`)
  } catch {
    return false
  }
}
...
function createAdapter(): BrowserAiSiteAdapter {
  const detectLoginState = async (page: Page): Promise<BrowserAiLoginState> => {
  ...
  return {
    site: 'generic-web' satisfies BrowserAiSite,

    matchesPage: (url, configuredUrl) => matchesConfiguredSite(url, configuredUrl),

    detectLoginState,

    openNewConversation: async (page, siteUrl) => {
      if (isPageClosed(page)) throw new Error('Browser page was closed.')
      if (!isSupportedBrowserAiSiteUrl(siteUrl)) throw new Error('The configured AI site URL is not supported.')
      ...
      const newConversation = await findVisibleLocator(page, [
        page.getByRole('link', { name: /new chat|new conversation|new thread|new session|start chat/i }),
        page.getByRole('button', { name: /new chat|new conversation|new thread|new session|start chat/i }),
      ])
      if (newConversation) {
        await newConversation.click().catch(() => undefined)
    ...
    waitForCompletion: async (page, timeoutMs, isCancelled) => {
      ...
      while (Date.now() < deadline) {
        ...
        if (SITE_ERROR_PATTERN.test(text)) throw new Error('The configured AI site reported a site or quota error.')
        if ((await detectLoginState(page)) === 'needs-login') throw new Error('Login is required for the configured AI site.')
      ...
      throw new Error(sawAssistantText ? 'The AI response did not stabilize before timeout.' : 'The AI response timed out.')
    },
...
export const genericWebAiAdapter = createAdapter()
