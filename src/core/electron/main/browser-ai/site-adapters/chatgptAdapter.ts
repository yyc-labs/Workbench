...
import type { BrowserAiAdapterStepUpdate, BrowserAiSiteAdapter, BrowserAiLoginState } from './browserAiSiteAdapter'
...
function createAdapter(): BrowserAiSiteAdapter {
  return {
    ...
    submitPrompt: async (page, prompt, onStep) => {
      onStep?.({ id: 'find-composer', status: 'active', message: 'Finding the ChatGPT composer.' })
      const composer = await findComposer(page)
      if (!composer) {
        onStep?.({ id: 'find-composer', status: 'failed', message: 'ChatGPT message composer was not found.' })
        throw new Error('ChatGPT message composer was not found.')
      }
