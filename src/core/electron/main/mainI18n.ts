...
const mainMessages = {
  'en-US': {
    tray: {
      ...
      quitApp: 'Quit Workbench',
    },
    migration: {
      title: 'Workbench data migration complete',
      message: 'Your data from IDE Electron has been migrated to Workbench.',
      detail: 'The old data directory is still kept at:\n{path}\n\nWould you like to delete it now?',
      deleteOldData: 'Delete Old Data',
      keepOldData: 'Keep Old Data',
      failedTitle: 'Workbench data migration needs attention',
      failedMessage: 'Workbench could not read the old IDE Electron data.',
      failedDetail: 'Close any running IDE Electron window, then restart Workbench to retry.\n\nPath: {path}\n\nError: {error}',
      ok: 'OK',
    },
    feishu: {
  ...
  'zh-CN': {
    tray: {
      ...
      openHome: '打开首页',
      openSettings: '打开设置',
      quitApp: '退出 Workbench',
    },
    migration: {
      title: 'Workbench 数据迁移完成',
      message: 'IDE Electron 中的数据已经迁移到 Workbench。',
      detail: '旧数据目录仍保留在：\n{path}\n\n现在要删除旧数据目录吗？',
      deleteOldData: '删除旧数据',
      keepOldData: '保留旧数据',
      failedTitle: 'Workbench 数据迁移需要处理',
      failedMessage: 'Workbench 无法读取旧的 IDE Electron 数据。',
      failedDetail: '请关闭正在运行的 IDE Electron 窗口，然后重启 Workbench 重试。\n\n路径：{path}\n\n错误：{error}',
      ok: '确定',
    },
    feishu: {
      unknownProject: '未知项目',
