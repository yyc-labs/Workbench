...
export function registerMarkdownDocumentIpcHandlers(service: MarkdownDocumentService, openRequestStore: MarkdownDocumentOpenRequestStore, routeMarkdownDocumentOpen: (markdownPath: string) => void): void {
  ...
  ipcMain.handle(IPC.MARKDOWN_DOCUMENT_HISTORY_REMOVE, (_event, filePath: string) => {
    openRequestStore.clearForPath(filePath)
    return service.removeHistory(filePath)
  })
  ...
  ipcMain.handle(IPC.MARKDOWN_DOCUMENT_ROUTE_OPEN, (_event, filePath: string) => {
    routeMarkdownDocumentOpen(filePath)
    return true
  })
}
